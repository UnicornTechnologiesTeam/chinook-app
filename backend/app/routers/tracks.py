...
def search_tracks(
    ...
    result = []
    for t in tracks:
        result.append(schemas.TrackDetail(
            TrackId=t.TrackId,
            Name=t.Name,
            UnitPrice=t.UnitPrice,
            artist_name=t.album.artist.Name if t.album else None,
            album_title=t.album.Title if t.album else None,
            genre_name=t.genre.Name if t.genre else None,
        ))
    return result
...
def get_track(track_id: int, db: Session = Depends(get_db)):
    ...
    return schemas.TrackDetail(
        ...
        artist_name=track.album.artist.Name if track.album else None,
        album_title=track.album.Title if track.album else None,
        genre_name=track.genre.Name if track.genre else None,
    )
